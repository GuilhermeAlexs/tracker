...
public class Main extends JFrame implements ChartMouseListener, MouseListener, ActionListener {
	...
	private void showNumberStatisticsPanel(Statistics stats){
		...
		labelDistance.setText("Dist�ncia: " + new DecimalFormat("#.##").format(stats.getLength()/(double)1000) + "km");
		
		labelElevation.setText("Eleva��o Min Avg Max: " + new DecimalFormat("#").format(stats.getMinElevation()) + "m  " + 
				new DecimalFormat("#").format(stats.getAvgElevation()) + "m  " + new DecimalFormat("#").format(stats.getMaxElevation()) + "m");

		labelElevationGainLoss.setText("Ganho/Perda de Eleva��o: " + new DecimalFormat("#").format(stats.getElevationGain())
				+ "m  " + new DecimalFormat("#").format(stats.getElevationLoss()) + "m");
		
		labelInclinationMax.setText("Inclina��o M�xima: " + new DecimalFormat("#.#").format(stats.getMaxInclinationPositive()*pC) + "%  "
				+ new DecimalFormat("#.#").format(stats.getMaxInclinationNegative()*pC) + "%");
		
		labelInclinationAvg.setText("Inclina��o M�dia: " + new DecimalFormat("#.#").format(stats.getAvgInclinationPositive()*pC) + "%  " 
	...
	protected void addButtonsToToolbar(JToolBar toolBar) {
	    ...
	    eraseSelectionsButton = makeNavigationButton("erase", ERASE_STRETCHS_EVT, "Apagar todas as marca��es da trilha");
	    ...
	    statisticsButton = makeNavigationButton("calc", STATISTICS_EVT, "Ver estat�stica");
	...
	public void chartMouseClicked(ChartMouseEvent e) {
		...
		if(firstClick){ //Usu�rio deu o primeiro click
			...
			escapeMouseMoveEvent = true; //Impede que movimentos do mouse apaguem a sele��o atual
	...
	public void mousePressed(MouseEvent e) {
		if(SwingUtilities.isRightMouseButton(e)){ //Bot�o direito sempre apaga a sele��o
	...
	private void eraseAllSelections(){
		int res = JOptionPane.showConfirmDialog(this, "Isso remover� todas as marca��es que voc� fez nessa trilha. Continuar?", "Remover Marca��es", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
	...
	private void insertCurrentTrailInDatabase(){
		...
		if(db.contains(trailName)){
			int res = JOptionPane.showConfirmDialog(this, "Essa trilha j� existe no banco. Deseja substituir?", "Substitui��o", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
		...
		JOptionPane.showConfirmDialog(this, "A trilha foi inserida com sucesso.", "Inser��o Finalizada", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);
	}
	
	private void deleteCurrentTrailInDatabase(){
		int res = JOptionPane.showConfirmDialog(this, "Voc� est� prestes a remover permanentemente essa trilha do banco. Continuar?", "Remo��o", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
...
/*	private void showInclinationSpeedGraphContinuous(){
		try {
			...
			System.out.println("Conversão para a Tabela Base concluída...!");
